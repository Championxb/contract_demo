  ...
  try {
    ...
    for (let i = 1; i <= 30; i++) {
      mockData.push({
        id: i.toString(),
        name: `软件开发服务合同${i}`,
        contractNo: `HT-2023-R${String(i).padStart(3, '0')}`,
        customerName: `客户${i}`,
        contractAmount: Math.floor(Math.random() * 2000000) + 500000,
        signDate: '2023-05-20',
        endDate: '2023-12-20',
...
const viewContract = (row) => {
  router.push(`/receipt-contracts/detail/${row.id}`)
}

// 编辑合同
const editContract = (row) => {
  router.push(`/receipt-contracts/edit/${row.id}`)
}
...
// 新增合同
const addContract = () => {
  router.push('/receipt-contracts/add')
}
